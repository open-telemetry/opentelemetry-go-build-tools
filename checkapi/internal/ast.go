	...
	for _, d := range f.Decls {
					...
					if structType, ok := t.Type.(*ast.StructType); ok {
						...
						if structType.Fields != nil {
							...
							for _, f := range structType.Fields.List {
								if len(f.Names) > 0 {
									fieldNames = append(fieldNames, APIstructField{Name: f.Names[0].Name, Type: ExprToString(f.Names[0].Obj.Decl.(*ast.Field).Type)})
								}
							}
						}
		...
		if fn, isFn := d.(*ast.FuncDecl); isFn {
			...
			if exported {
				...
				if !fn.Name.IsExported() && len(f.ReturnTypes) == 1 && f.ReturnTypes[0] == "component.Config" {
					if ret, ok := fn.Body.List[len(fn.Body.List)-1].(*ast.ReturnStmt); ok {
						if len(ret.Results) == 1 {
							...
							default:
								panic(fmt.Sprintf("[%s] Unsupported type %T", f, r))
							}
						}
					}
