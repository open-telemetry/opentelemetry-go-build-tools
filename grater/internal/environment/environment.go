...
// Package environment holds utilities to work with an instance of an environment to run tests.
package environment
...
// Environment struct initializes an environment to run tests.
...
func (env *Environment) RunTests(ctx context.Context, mainModuleBase, mainModuleHead module.Module, dependents []module.Module, replacements [][]module.Module) ([][]container.ExecuteCommandResponse, error) {
	CleanupMainModule, mainModuleBinds, err := env.getMainModuleBinds(ctx, mainModuleHead)
	if err != nil {
		return nil, err
	}
	defer CleanupMainModule()

	CleanupReplacementModule, replacementBinds, err := env.getReplacementBinds(ctx, replacements)
	if err != nil {
		return nil, err
	}
	defer CleanupReplacementModule()
